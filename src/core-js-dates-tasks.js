...
function getNextFriday(date) {
  ...
  if (day > 5) numberOfDays = 6;
  return new Date(ms + numberOfDays * 24 * 60 * 60 * 1000);
}
...
function getCountDaysInMonth(month, year) {
  ...
  if (month === 2 && isLeap) return days[month - 1] + 1;
  return days[month - 1];
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (end - start + 1 * 24 * 60 * 60 * 1000) / (1000 * 60 * 60 * 24);
}
...
function formatDate(date) {
  ...
  const dateAsString = dateObj.toLocaleString('en-US', {
    timeZone: 'UTC',
    hour: 'numeric',
    minute: 'numeric',
    second: 'numeric',
...
function getWeekNumberByDate(date) {
  ...
  tempDate.setDate(tempDate.getDate() - firstWeekLength);
...
function getNextFridayThe13th(date) {
  ...
  while (start.getDay() !== 5 || start.getDate() !== 13) {
